...
import { updateOrderAsAdmin, getPaymentMethodById, getDeliveryMethodById } from "../../../api/OrderAPI"
...
const AdminOrderEditPage: React.FC = () => {
  ...
  const [deliveryMethodName, setDeliveryMethodName] = useState<string>("")
  ...
  useEffect(() => {
    const testApi = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:8080/api/orders/admin/${testId}`, {
          headers: {
  ...
  useEffect(() => {
    const token = localStorage.getItem("token")
    if (!token) {
      toast.error("Please log in to edit orders")
      navigate("/login")
      return
    }
    ...
    const fetchOrderDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8080/api/orders/admin/${orderId}`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        try {
          const paymentMethod = await getPaymentMethodById(orderData.paymentId)
          setPaymentMethodName(paymentMethod.nameOfPayment)
        } catch (error) {
          console.error("Error fetching payment method:", error)
          setPaymentMethodName(`ID: ${orderData.paymentId}`)
        }

        // Fetch delivery method name
        try {
          const deliveryMethod = await getDeliveryMethodById(orderData.deliveryMethodId)
          setDeliveryMethodName(deliveryMethod.nameOfDeliveryMethod)
        } catch (error) {
          console.error("Error fetching delivery method:", error)
          setDeliveryMethodName(`ID: ${orderData.deliveryMethodId}`)
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orderId = Number.parseInt(id)
      await updateOrderAsAdmin(orderId, deliveryStatus, paymentStatus, addressOfBuyer)
        ...
        <div className="col-lg-8">
          <div className="card mb-4">
            <div className="card-header bg-primary text-white">
              <h4 className="mb-0">Edit Order #{order.id}</h4>
            </div>
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-3">
                  ...
                  >
                    ...
                    <option value={DeliveryStatus.SHIPPED}>Shipped</option>
...
const AdminOrderEditPage_Admin = RequireAdmin(AdminOrderEditPage)
export default AdminOrderEditPage_Admin
